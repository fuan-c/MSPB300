...
namespace dao
{
    class frmMSAD009 : dao.daoBase
    {
        ...
        //コンストラクタ
        public frmMSAD009(string name, string userid)
        {
            dbConnection(name);
            _userId = userid;
        ...
        /// <return>none</return>
        public void update_T_SHIPPING_REQUEST(string CONTROL_NO, string staff_name)
        {
            try
            {
                _logger.Info("出処依頼テーブル更新 開始");
                _logger.Info("管理番号 [" + CONTROL_NO + "]");
                _logger.Info("担当者名 [" + CONTROL_NO + "]");
                ...
                sb.AppendLine("    SHIP_STOP_RECORD = TO_CHAR(SYSDATE,'YYYYMMDD') ");
