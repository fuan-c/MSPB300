...
namespace dao
{
    class frmMSAD001 : dao.daoBase
    {
        ...
        //コンストラクタ
        public frmMSAD001(string name, string userid)
        {
            dbConnection(name);
            _userId = userid;
        ...
        //出荷依頼テーブルより管理番号の取得
        public DataTable select_CONTROL_NO_From_T_SHIPPING_REQUEST(string control_no)
        {
            ...
            _logger.Info("管理番号 取得開始");

            sb.AppendLine("SELECT CONTROL_NO ");
            sb.AppendLine("FROM T_SHIPPING_REQUEST ");
            sb.AppendLine("WHERE CONTROL_NO = ");
        ...
        public void insert_T_SHIPPING_REQUEST( string staff_name, int next_no, string list)
        {
            try
            {
                ...
                _logger.Info("出荷依頼ファイル　登録1行イメージ [" + list + "]");
                ...
                sb.AppendLine("   ,CONTRACTOR_INFO_ID ");
                sb.AppendLine("   ,RESEPTION_DATE ");
                sb.AppendLine("   ,REGIST_DATE ");
                sb.AppendLine("   ,REGIST_NAME ");
                ...
                //発送先郵便番号の設定
                //■「発送先区分」＝0の時、ハイフンなし　「契約者郵便番号」をセット、
                //ただし、「記名被保険住所」がある時は、「記名被保険者郵便番号」をセット
                //■「発送先区分」＝1　or 2 の時、「代理店個別発送郵便番号」をセット
                if (!string.IsNullOrEmpty(fields[21]) && fields[21] == "0")
                {
                    if (!string.IsNullOrEmpty(fields[18]))
                    {
                        sb.AppendLine("  ,'" + fields[17] + "'");
                    }
                    else
                    {
                        sb.AppendLine("  ,'" + fields[13] + "'");
                    }
                }
                else if (!string.IsNullOrEmpty(fields[21]) && (fields[21] == "1" || fields[21] == "2"))
                {
                    sb.AppendLine("  ,'" + fields[24] + "'");
                }
                //発送先住所の設定
                //■「発送先区分」＝0の時、全角60文字　「契約者住所」セット、
                //ただし、「記名被保険者住所」がある時は、「記名被保険者住所」をセット。
                //■「発送先区分」＝1　or 2 の時、「代理店個別発送住所」をセット。
                //データセットの際、ブランクの連続は、１つにする。
                if (!string.IsNullOrEmpty(fields[21]) && fields[21] == "0")
                {
                    if (!string.IsNullOrEmpty(fields[18]))
                    {
                        sb.AppendLine("  ,'" + System.Text.RegularExpressions.Regex.Replace(fields[18], @"\s+", "　") + "'");
                    }
                    else
                    {
                        sb.AppendLine("  ,'" + System.Text.RegularExpressions.Regex.Replace(fields[14], @"\s+", "　") + "'");
                    }
                }
                else if (!string.IsNullOrEmpty(fields[21]) && (fields[21] == "1" || fields[21] == "2"))
                {
                    sb.AppendLine("  ,'" + System.Text.RegularExpressions.Regex.Replace(fields[25], @"\s+", "　") + "'");
                }
                //発送先電話番号の設定
                //■「発送先区分」＝0の時、半角数字16文字　「契約者電話番号」をセット、
                //ただし、「記名被保険者住所」がある時は、「記名被保険者電話番号」をセット。
                //■「発送先区分」＝1　or 2 の時、「代理店個別発送電話番号」をセット
                if (!string.IsNullOrEmpty(fields[21]) && fields[21] == "0")
                {
                    if (!string.IsNullOrEmpty(fields[18]))
                    {
                        sb.AppendLine("  ,'" + fields[20] + "'");
                    }
                    else
                    {
                        sb.AppendLine("  ,'" + fields[15] + "'");
                    }
                }
                else if (!string.IsNullOrEmpty(fields[21]) && (fields[21] == "1" || fields[21] == "2"))
                {
                    sb.AppendLine("  ,'" + fields[27] + "'");
                }

                sb.AppendLine("  ,SUBSTR('" + fields[0] + "', 1, 8)");
                sb.AppendLine("  ,SUBSTR('" + fields[0] + "', 9, 6)");
